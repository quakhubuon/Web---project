...
<%
		...
		int count = productDAO.getTotalProductS(name);
		int endPage = count / 3;
		if (count % 3 != 0) {
			endPage++;
		}
		List<Product> list = productDAO.pagingProductS(name, index);
	...
	<section class="dishes" id="dishes">
		...
		<div id="pagination">
				<c:forEach begin="1" end="${endP}" var="i">
					<c:if test=""></c:if>
					<a class="page-item" href="searchname.jsp?tukhoa=<%=name%>&index=${i}">${i}</a>
				</c:forEach>
		</div>
	</section>
